...
const parseUrl = ({ location, days }: ForecastQueryKey) =>
    `/forecast.json?key=${getApiKey()}&q=${location}&days=${days}`;
...
export const useForecastData = (location: string, days: number = 10): UseQueryResult<WeatherData> =>
    useQuery({
