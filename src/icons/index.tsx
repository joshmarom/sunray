...
const getFileName = (name: ConditionText) => {
    const iconName = Object.entries(conditionIcons).find(([key]) =>
            key.trim().toLowerCase() === name.trim().toLowerCase())?.[1];

    return iconName ? `${iconName}.svg` : undefined;
};

const getIds = (svg: Document): ReadonlyArray<string> =>
    [...svg.querySelectorAll('symbol[id], linearGradient[id]')].map(({ id }) => id);
